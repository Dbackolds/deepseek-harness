...
const TOOL_PACKAGES: ToolPackage[] = [
  ...
  },
  {
    ...
    requires: ['ctx.tools', 'ctx.fs', 'ctx.systemPrompt', 'ctx.attachments (media-tool registration)', 'ctx.llm + an image/video-capable route (media-tool execution)'],
    writes: ['tool/call', 'fs/write-intent or fs/edit-intent for mutations', 'fs/observed after read presence/absence or successful file operation', 'durable attachment (read_image/read_video)', 'tool/result'],
    async mount(ctx) {
      ...
      // the attachments-conditional media schemas without attachment I/O.
      await ctx.plugin(LocalFileSystem)
      await ctx.plugin(CatalogAttachmentStore)
      await ctx.plugin(ToolFs)
    },
    note:
      'The read-before-write/edit policy is added by `@deepseek-ai/dsh-fs-observation-policy` (an `fs/*` event-gate plugin, no schema change); a deployment that loads these tools is expected to also load it. The image and video tools are not registered without `ctx.attachments`; their schemas are route-independent, and execution refuses unless the exact routed model declares image (respectively video) input.',
  },
  {
