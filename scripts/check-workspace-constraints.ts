...
/** Checkout-only app shells that must stay private and out of the publish set. */
...
const packageFileExtras: Readonly<Record<string, readonly string[]>> = {
  ...
  '@deepseek-ai/dsh-host-plugin-catalog': ['catalog.json'],
  '@deepseek-ai/dsh-client-ui-settings-plugin-marketplace': ['lib/reboot-watchdog.js'],
...
function expectedDshPackageFiles(manifest: PackageManifest): readonly string[] {
  ...
  return [
    ...
    // Dual-face marketplace: the Host row imports ./host so the browser row
    // can keep the empty package entry without registering /plugin-marketplace twice.
...
function checkWorkspace({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  } else if (privateAppDirectories.has(dir)) {
    if (manifest.private !== true) {
      errors.push(`${label}: private app must set "private": true`)
    }
