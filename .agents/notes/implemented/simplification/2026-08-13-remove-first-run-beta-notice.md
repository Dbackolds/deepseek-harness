...
This removal eliminated the full-viewport notice and its telemetry copy. The later restoration is intentionally a different presentation and copy revision: a shared modal, the remote scenario again covering process-local acknowledgement, and the existing `welcomeNoticeVersion` field recording the new copy version. The historical telemetry prompt remains absent, and the later [credential-dialog removal](2026-08-15-remove-first-run-api-key-dialog.md) leaves the notice as the only first-run step.
