...
这次移除消除了占满视口的声明及其遥测文案。后续恢复有意采用不同的展示与文案版本：共用弹窗、远程场景重新覆盖进程内确认，既有 `welcomeNoticeVersion` 字段记录新的文案版本。历史上的遥测提示仍未恢复，后续的[凭据弹窗移除](2026-08-15-remove-first-run-api-key-dialog.zh.md)使该声明成为唯一的首启步骤。
