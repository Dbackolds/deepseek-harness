...
- `LlmDiscoveredModel` makes every field but `id` optional, because most listings disclose an id and nothing else. When a listing or catalog entry discloses selectable thinking levels, those ride optional `reasoningEfforts` and `supportsReasoningEffort`. The reply is candidates, not a catalog: a surface adopting one still owes the capacities the adapter requires.
