...
- `LlmDiscoveredModel` 除 `id` 外每个字段都可选，因为大多数列表只公布 id。当列表或 catalog 条目公布了可选思考档位时，这些档位会随可选的 `reasoningEfforts` 与 `supportsReasoningEffort` 返回。回复是候选而非 catalog：采纳其中一条的界面仍要补上适配器所需的容量。
