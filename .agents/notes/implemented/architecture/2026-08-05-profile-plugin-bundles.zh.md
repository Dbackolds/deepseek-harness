...
随附的组合包是 `@deepseek-ai/dsh-base`（共享核心配置行）、`@deepseek-ai/dsh-web-app`（浏览器 Host 配置行与 Web 运行时粘合层）和 `@deepseek-ai/dsh-headless`（直接叠加在 base 上且不含 web-app 的一次性 runner）。通用的 `dsh --profile <name>` 把剩余参数交给该 profile 的命令行启动行：Web 持有自己的 flag ，headless 则持有任务位置参数。patch overlay 使用启动器持有的 `--patch`。`dsh plugin --profile <name> <args...>` 是一层薄薄的 pnpm 转发器，负责初始化 profile，并依据已安装包的组合包声明调和 `dsh.profile.bundles`；没有组合包声明的包保持为普通依赖。调和、组合包探测、pnpm 启动以及用户层读写都位于 `@deepseek-ai/dsh-app-boot`（`packageExportsBundle`、`reconcileProfilePlugins`、`runProfilePnpm`、`readProfilePatches`、`writeProfilePatches`），因此 Host 插件可以改同一批文件。profile 启动通过 `provideProfile`，在与 `ctx.cmdlineArgs` 相同的 `prepare` 钩子里把该身份发布为 `ctx.profile`（`name`、`dir`、`installAnchor`）。[Headless 作为直接 core 入口](2026-08-09-headless-direct-core-entry-point.zh.md)负责 headless 组合约定。
...
- **在组合包 manifest 中放一个启动前 `context` 模块**承载启动期取值（dist 路径、flag 事实）：否决，改用纯插件——粘合逻辑就是普通配置行和由应用持有的启动服务，因此组合始终可完整 dump，manifest 保持纯数据。启动器提供的宿主 slot（`ctx.cmdlineArgs`、`ctx.appExit`、`ctx.profile` 与环境快照）在任何配置树条目挂载之前，于 `boot()` 的 `prepare` 钩子中提供。
- **只把 profile 变更留在 `apps/cli`**：CLI 转发器仍持有 argv 路径锚定，但要安装或开关包的 Host 插件无法导入 `apps/cli`。因此这些 helper 放在已经持有磁盘 profile 格式的 `dsh-app-boot` 上。
...
- `apps/cli` 收缩为 argv 解析、profile 机制的消费方和 pnpm 转发器；`AppCLIEntry` 与各表层专属的启动路径全部移除。注入 `profile` 的树外 Host 插件可以安装、移除或开关同一个 profile，而不必再复制一份逻辑。
