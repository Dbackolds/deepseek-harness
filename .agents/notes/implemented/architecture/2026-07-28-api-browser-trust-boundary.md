...
- Settings describe/update/replace/mutate ride a declared `trustedHosts` authority with ordinary `/api` reads. Native `settings.openDocument`, credentials, and host dialogs stay loopback-only. The [Host-backed preferences note](../bug-fix/2026-08-06-host-backed-web-preferences.md) owns that split.
