...
- `settings.describe`/`update`/`replace`/`mutate` 与普通 `/api` 读取一样，对已声明的 `trustedHosts` 授权开放。原生 `settings.openDocument`、凭据和宿主对话框仍仅限回环。该拆分由 [Host settings 支撑的偏好决策](../bug-fix/2026-08-06-host-backed-web-preferences.md) 拥有。
