...
`@deepseek-ai/dsh-session-control` publishes `ctx.sessionControl` as a trusted in-process directory. `search()` and `get()` list the live-preferred corpus from `ctx.sessionQuery.listSessions()`, attach each row's latest title and live Agent status (`running` / `idle` / `ready`), and filter a case-insensitive substring against session id, cwd, and title. `stop()` cancels a live Agent with `keepInbox: true` and treats a known identity without a driver as an accepted no-op. `send()` delivers one non-empty text block through `followup()` or `steer()` and records source `{ kind: 'plugin', plugin: 'session-control' }`.
