...
专用的随包提供方可以贡献不可变的 skill，无需文件系统发现。交付的 CLI（命令行界面）默认将 `@deepseek-ai/dsh-skill-badge` 声明为禁用；启用其组合配置行，就会通过同一个注册表和消费方贡献官方徽章指令（见[决策](2026-08-06-bundled-dsh-badge-skill.md)）。已发布的 base 组合还会挂载 `@deepseek-ai/dsh-skill-session-control`，贡献 `dsh-session-control` 指令，用于平级会话的搜索、停止、发送、改名和库管理工具（见[决策](2026-08-19-bundled-session-control-skill.md)）。
