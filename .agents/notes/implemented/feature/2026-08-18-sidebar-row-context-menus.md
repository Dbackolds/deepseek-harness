...
A real Workspace row and a non-blank Session row each own two `Menu` instances. The trailing ellipsis keeps its original icon dropdown inside the hover-only `rowActions` cell. The Session context menu is a separate standard-size text-only list at the pointer: Pin task, Rename task, Archive task, Mark as unread, Open in split view, then Reveal in Explorer and the copy-path rows. Pin stores the Session in a browser-local pinned set and renders it in a global Pinned section above the Workspace list. A pinned row offers Unpin task instead of Pin task. Mark as unread restores the Completed reminder through `sessions.markUnread`. Open in split view opens the Session. Copy log path writes the browser-local `~/.dsh/sessions` JSONL guess. Reveal in Explorer and the cwd copy rows use the session's projected directory. Opening one menu closes the other. Hover cards stay suppressed while either menu is open. The live GUI serves `lib/client.js`, not sources; `/reboot` restarts the session and does not rebuild or swap that bundle.
