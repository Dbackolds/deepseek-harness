...
Every ordinary runtime root is the implicit Lead of a Team identified by that root's `SessionId`. The Team has no creation event: its Lead pseudo-row exists by identity, while durable state begins with the first member, message, or task event. A roster is flat and contains at most the configured number of immutable lowercase-kebab-case names. Each teammate is a continuable direct child with a reserved Session id; only the Lead creates or interrupts teammates. Ordinary provider-owned subagents outside the roster are not Team members: `tryMembership` classifies them by `SessionHeader.origin === 'subagent'` at publication, with a suffix `subagent/descriptor` as fallback for logs that omit origin. A one-shot provider appends that descriptor only after the first prompt assembly, so origin is the classification that exists when `agent/created` installs scoped Team tools. An ordinary fork is a new root whose inherited Team records are excluded by their ancestor `TeamId`. The [one-shot membership classification](../bug-fix/2026-09-07-one-shot-subagent-team-membership.md) owns that origin-before-descriptor timing.
