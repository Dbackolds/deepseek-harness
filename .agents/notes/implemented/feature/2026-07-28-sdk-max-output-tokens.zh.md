...
进程内 subagent 继承父级的提供方、模型、输出上限，以及相同路由上的推理强度。显式的 `SubagentStartRequest.agentOptions.maxTokens` 或 `reasoningEffort`（包括通过 `dsh-tool-subagent` 配置的值）会覆盖该子级及其后代的继承值。子 agent 更换提供方或模型时，不会继承上一模型的不透明推理强度。进程外提供方自行持有其独立运行时的配置；因此 `subagent-dsh-sdk` 公开独立的可选 `maxTokens`，并通过该子运行时自己的 SDK 握手传入。
