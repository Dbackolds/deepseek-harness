...
A new Workspace takes its display name from the directory it was created in. Distinct canonical paths may share the same basename-derived title ([identity decision](../bug-fix/2026-07-31-same-basename-workspace-adoption.md)); the explicit rename operation retains its duplicate-title check. Separate display-name and directory-name inputs remain outside this flow. Mid-conversation Session rehome (same conversation, new effective home and workspace account, including the No Repo start) is specified in [Session rehome](../../proposed/feature/2026-08-20-session-rehome.md). Manual adoption of Ungrouped historical sessions remains outside this flow.
