...
每个 Workspace 持久化一项浏览器本地打开状态：关闭表示零条 Session 行，打开表示未折叠的各活动分区，其中历史记录在点展开其余之前仍最多五条。存在更多历史记录时，**展开其余**只在当前挂载期间显示剩余历史记录；关闭整个 Workspace 会清除此临时展开，因此重新打开时恢复为五条历史记录。只有在用户尚未为该 Workspace 存储明确状态时，当前 Session 所在分组才会自动打开。从 Workspace 行创建 Session 时会在启动 Session 前打开目标分组，使状态传播完成后新行保持可见。就绪的 Workspace 基线发生变化后，浏览器会移除基线中不存在 id 的展开状态、顺序和已观察时间戳记录，同时保留 Ungrouped 和单列表记账。
...
- 打开 Workspace 不会隐藏已完成或运行中行；未执行明确的**展开其余**手势时，历史记录最多显示五条；关闭分组只重置这项临时历史记录折叠。
