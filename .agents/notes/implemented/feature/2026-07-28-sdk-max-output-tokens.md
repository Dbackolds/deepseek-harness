...
In-process subagents inherit the parent's provider, model, output cap, and same-route reasoning effort. An explicit `SubagentStartRequest.agentOptions.maxTokens` or `reasoningEffort`, including one configured by `dsh-tool-subagent`, overrides the inherited value for that child and its descendants. A child that changes provider or model does not inherit the previous model's opaque effort id. Out-of-process providers own the configuration of their separate runtime; `subagent-dsh-sdk` therefore exposes its own optional `maxTokens` and forwards it through that child runtime's SDK handshake.
