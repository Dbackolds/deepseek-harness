...
Every user, steering, and assistant message row renders its event clock as a trailing, always-visible `<time>` label. `MessageClock` formats `node.time` through `formatMessageClock` and sits on the message band itself: the user/steering bubble keeps the clock to the right of the bubble, and assistant narration keeps it to the right of the markdown body. Same calendar day prints `h:mm` plus a locale meridiem (`上午` / `下午`, `AM` / `PM`); earlier this year prefixes the `clock.md` date template; other years prefix `clock.ymd`. `useCalendarDay` still widens the label after local midnight. Pending steering has no durable event time, so it mounts no clock.
