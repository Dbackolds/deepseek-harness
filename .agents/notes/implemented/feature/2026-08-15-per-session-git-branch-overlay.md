...
The Web chip fills `conversation.hero.branch` beside the existing hero chips. It talks to `git.describe` / `git.checkout` / `git.createBranch`. The chip loads `git.describe` only when the current session id or workspace id changes; overlapping loads for that identity coalesce, and a newer identity aborts the older describe. A workspace that is not a Git checkout hides the chip. The picker lists local short names and remote-tracking names that include the remote (`origin/master`); it does not collapse a remote-tracking name onto a same-named local head. A detached workspace HEAD reports `detached: true`. The chip and current-row title then say Detached HEAD rather than the short object id; `currentBranch` still keeps that id so a later checkout can reuse it. The current-branch row also shows this session worktree's uncommitted path count and, when an upstream exists, the unpushed commit count.
