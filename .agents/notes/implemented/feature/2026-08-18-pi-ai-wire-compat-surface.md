...
`thinkingFormat: zai` without `supportsDeveloperRole` fills `false` at materialization. Official zai catalog entries refuse that role; a private URL still detects as OpenAI, so a profile that names only the zai thinking format would otherwise leave a reasoning model's system prompt on `developer`. An explicit `true` still wins.
