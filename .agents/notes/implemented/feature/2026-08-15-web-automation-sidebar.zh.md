...
创建停留在服务已接受的四种选择器上。内置模板只预填该表单，提交前不持久化规则。保持唤醒开关写入 Host 的 `ui-automation.keepAwake`；存活的宿主随后持有操作系统休眠断言，关掉开关或卸载插件时释放。表单不解析自然语言，也不编辑已有选择器。
...
包测试覆盖 slot 注册与 HMR 拆除、store 的 list/create/enable/run-now/delete 与页面开合、选择器摘要、草稿校验、保持唤醒偏好与操作系统持有、模板叠加，以及触发器加中间栏整页。Web e2e 场景通过真实 Host 创建一条 `after` 规则，并在整页里断言它。侧栏外壳快照包含空的 `sidebar.automation` hole。
