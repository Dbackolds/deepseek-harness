...
`ui-sidebar` 在 New Session 下方声明 `sidebar.automation`。`@deepseek-ai/dsh-client-ui-automation` 占据该 seat，触发器打开一个走现有 `automation.*` 线路的模态框：列出、创建（恰好一种 after / at / every / local-clock）、启用、立即运行和删除。Host 仍是事实来源。面板不新增 Settings 导航页，因为产品请求是 New Session 旁边的侧栏入口；第二个家会拆开同一批集合。
...
包测试覆盖 slot 注册与 HMR 拆除、store 的 list/create/enable/run-now/delete、选择器摘要、草稿校验，以及触发器/列表/创建面板。Web e2e 场景通过真实 Host 创建一条 `after` 规则，并在面板里断言它。侧栏外壳快照包含空的 `sidebar.automation` hole。
