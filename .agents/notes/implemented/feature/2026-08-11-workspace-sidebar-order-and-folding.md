...
Each Workspace persists one browser-local open state: closed means zero Session rows and open means every live Session plus idle Sessions capped at five rows until Show more. When more idle Sessions exist, **Show more** reveals the remaining idle rows only for the current mount; closing the whole Workspace clears this transient expansion, so reopening returns to five idle rows. Live work never enters that overflow; [Sidebar live work without status folders](2026-08-19-sidebar-live-idle-without-status-folders.md) owns the live/idle split. The current Session's group opens automatically only when the user has not already stored an explicit state for that Workspace. Creating a Session from a Workspace row opens the target group before starting the Session, keeping the new row visible when state propagation completes. After a ready Workspace baseline changes, the browser removes expansion, order, and observed-timestamp records for ids absent from that baseline while retaining the Chat, flat-list, and Hidden-section accounts. Chat is always present as the trailing grouped bucket ([Sidebar Chat Bucket](2026-08-21-sidebar-chat-bucket.md)). Hidden Workspaces stay on the Host order and session account; the grouped list folds them into the trailing Hidden section ([Hide Workspace](2026-08-20-workspace-hide.md)).

The combined view menu offers **Manual** and **Last updated** in grouped and flat presentation, with one browser-local persisted order per account. A real project Workspace initializes from `WorkspaceView.sessionIds`; Chat and the cross-Workspace flat list initialize from recency. Chat writes the No Repo Host account when that workspace is registered. Entering Last updated performs one complete recency sort; a later user prompt or steer promotes that Session once, and dragging may edit the resulting order. Returning to Manual preserves the current order and only disables later activity promotion. Manual-mode drags for a real project Workspace also write the Host Session account, while Chat without No Repo and flat-list drags and activity promotion remain browser-local. Flat rows omit an empty leading status slot because they have no parent hierarchy, while a visible status retains its slot.
...
- Workspace order is durable and shared through the Host, while grouping, open state, per-account Session view order, and query state remain browser-local presentation preferences. Chat and the flat list support the same drag and promotion rules; Chat writes the No Repo Host account when that workspace is registered, and otherwise stays browser-local.
...
- Opening a Workspace never hides live rows, never shows more than five idle Sessions without an explicit **Show more** gesture, and closing it resets only that transient idle fold.
