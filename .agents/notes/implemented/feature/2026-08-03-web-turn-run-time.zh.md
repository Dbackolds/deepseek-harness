...
事件时钟不再属于这一行。每条已落盘消息自己绘制始终可见的行尾时钟（[决策](2026-08-20-web-always-visible-message-clock.zh.md)）。已结算的运行时长、TTFT（首 token 延迟）与吞吐仍留在 IconActions 行，有值即可见。复制与分支图标始终可见。
...
**将整个操作行隐藏至悬停时才显示。** 复制与分支是值得让用户发现的操作入口，而整行级别的显隐切换有布局偏移的风险。事件时钟现在位于消息行上；这条页脚只还承载被动的指标文本。

## 后果

轮次时长在运行中和结束后都可见，且不需要新的会话事件；两处读数共用精确的日志边界和格式化方式。结束后的时长包括最后一条 assistant 文本之后、直至 `turn/end` 的活动；若 `turn/start` 位于已加载窗口之外，则不显示标签。持续跳动的活动时钟只保留视觉呈现，不会被重复播报。
