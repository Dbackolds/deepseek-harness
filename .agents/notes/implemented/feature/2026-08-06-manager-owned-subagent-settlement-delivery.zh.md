...
空闲父级得到一个普通的后续轮次。繁忙父级在发送时遵循 Host `subagent-delivery.settlementBusy`：`steer`（默认）把通知准入到最近的后续 step，因为 `Inbox.claim()` 会在一个边界上整批取走 next-step；`queue` 则开启后续一轮。采用 steer 而非 inject 仍是正确性默认——驱动运行期间该唤醒是空操作，同时它关闭了「驱动在状态读取与发送之间退出」的那个窗口。该字段是用户选择投递位置，不是可以省略通知的部署开关。拆卸注入保持固定。
...
**把投递做成可省略。** 可以让通知缺席的部署开关会把面向模型的文本重新变回「通常」，而这正是本次改动要消除的失效。Host `subagent-delivery.settlementBusy` 可以选择最近一步或后续一轮，但不能丢掉通知。拆卸注入保持固定。
