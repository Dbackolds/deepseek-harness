...
DeepSeek Harness 仍处于早期阶段，并在积极开发中。开 PR（Pull Request）请遵循[开 PR 实操手册](docs/cookbook/opening-a-pull-request.zh.md)。贡献代码不是唯一帮忙方式：
...
DeepSeek Harness 的设计支持深度定制。本仓库中的包并不天然比社区开发的包更重要。你可以将本仓库看作一种理念、一份示例以及一处灵感来源，而不是要求社区遵循的方向。
