...
- **Dispose to quiescence** — teardown fails in-flight runs as `abort` and AWAITS each worker's exit before resolving. Caller abort reasons, including typed `AgentCancelCause` objects, are rendered through `formatAbortReason` rather than `String(reason)`.
