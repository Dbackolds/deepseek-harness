...
describe('catalog-route model discovery', () => {
  it('answers from the installed registry, with capacities and no network call', async () => {
    ...
    expect(flash?.reasoningEfforts).toEqual({ off: null, low: 'low', high: 'high', max: 'max' })
...
describe('draft-provider model discovery', () => {
  ...
  it('drops unknown string tokens and an empty effort list', async () => {
    ...
    await expect(ctx.llm.discoverModels('llm-pi-ai', { baseURL: server.url })).resolves.toEqual([
      { id: 'mystery-only', name: 'mystery-only' },
      { id: 'empty-list', name: 'empty-list' },
      { id: 'off-only', name: 'off-only', reasoningEfforts: { off: null } },
      { id: 'high-label', name: 'high-label', reasoningEfforts: { high: 'high' } },
    ])
  })
