...
Bundled skill provider that contributes `dsh-session-control` to `ctx.skills`. The skill tells the model when and how to use the `session_control_*` tools to search every logical session, stop a turn, or deliver a later message.
...
- The skill does not resume a cold session. A storage-only identity still requires an owner that keeps the `AgentHandle`.
