...
内置 skill 提供方，向 `ctx.skills` 贡献 `dsh-session-control`。该 skill 告诉模型何时以及如何使用 `session_control_*` 工具，以搜索全部逻辑会话、停止一轮或投递后续消息。
...
- 该 skill 不恢复冷会话。仅存于存储的身份仍需要会保留 `AgentHandle` 的所有者。
