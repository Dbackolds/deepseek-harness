...
function webSurfacePrompt(webUrl: string): string {
  const updateContract = 'The client-plugin HMR receiver is active, but automatic no-refresh reloads happen only while Settings → Automatic hot reload is on and '
    + '`pnpm run dev:web` is also running from this same checkout to rebuild their bundles; verify both before promising automatic updates. '
    + 'Otherwise rebuild the affected Web artifacts and use Settings → Reload plugins, or refresh this existing URL. '
    + 'Every other change — the apps/web shell and plain packages — still requires rebuilding the affected Web artifacts and verifying this existing URL after a page refresh. '
...
export function apply(ctx: Context, config: Config): void {
  ...
  if (config.printUrl) {
    // Desktop and other supervisors open the window on this line. Print as
    // soon as this row can form the loopback URL: the HTTP server is already
    // listening, unmatched paths stay 404 until later rows register, and the
    // browser boot kernel waits for client plugins itself.
    console.log(`dsh web: ${localWebUrl(ctx)}${runtime.lanAddresses[0] === undefined ? '' : ` (LAN: http://${runtime.lanAddresses[0]}:${String(ctx.webServer.port)})`}`)
  }
}
