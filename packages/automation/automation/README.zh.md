...
- **仅 Host 进程投递** — 规则只在本 Web Host 存活时准时开火；关掉的 desktop 窗口不会做操作系统级唤醒。取消当前等待（包括 `create` / `update` / `delete` / `runNow` 或 idle overlap watcher）会结束该次等待并重新推导下一目标。休眠中的 Host 至少每分钟按墙上时钟复核到期规则，因此错过的 local-clock 时刻会在醒来后开火，而不是把原延迟睡完。被拒绝的 `fireDue` 会记日志，owner 继续处理后续到期规则，然后等待一分钟再重试仍到期的目标。
