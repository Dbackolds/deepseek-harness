...
The replayed system prompt, tools, and shadowed-region messages match the conversation's last routed request byte-for-byte, so the provider's warm prefix cache is reused up to the trailing instruction; only that instruction, and the summary output, is uncached. Same-route effort is part of that prefix-matching request. Routing the summarizer to a different provider/model, or compacting a non-head range, forgoes this reuse.
