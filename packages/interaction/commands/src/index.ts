...
export class CommandRuntime extends TypertRemoteService {
   ...
   * for deferred collection. Videos settle unconditionally: no command
   * declares video input, so every submitted video is refused before any
   * admission work.
   *
  ...
  async execute(
    ...
    if (submittedAttachments.some(part => part.type === 'video')) {
