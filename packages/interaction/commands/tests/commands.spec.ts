...
describe('command attachments', () => {
  ...
  it('settles videos sent to any command as a logged error before the handler', async () => {
    const ctx = await mount()
    const { agent } = await mintAgentScope(ctx, 'a')
    const handler = vi.fn(() => ({ kind: 'success' as const }))
    ctx.commands.register(accepting(handler))
    const execution = await ctx.commands.execute(
      agent, '/vision x', [{ type: 'video', mediaType: 'video/mp4', data: 'AAAA' }], new AbortController().signal)
    expect(execution?.result).toEqual({ kind: 'error', text: '/vision does not accept video attachments' })
    expect(handler).not.toHaveBeenCalled()
    expect(lifecycleOf(agent)).toMatchObject([
      { type: 'command/run', data: { name: 'vision' } },
      { type: 'command/done', data: { kind: 'error', text: '/vision does not accept video attachments' } },
    ])
  })
