...
- `Workspace.attachSession(id)` — validates a live session's effective home (`workspace/home` or `git/worktree`, else header cwd) against the workspace path and prepends a new id. Cold attach still uses birth cwd. Unknown sessions, absent/unresolvable/non-directory cwd values, and mismatches reject without writing. `detachSession` removes only the candidate index entry.
