...
`AgentOptions` supplies the initial provider/model route, an optional positive `maxTokens` output cap, and an optional adapter-owned `reasoningEffort`. The concrete loop resolves any exact-model adapter default, records the effective values in the request header, and applies them to each conversation-model request; an explicit Agent option wins, while omission leaves the adapter or provider route default in control.
...
- `agent.inbox` — the agent-owned projection of durable `agent/inbox/spliced` events. `nextTurn` and `nextStep` expose pending `UserMessage` values. `append`, `prepend`, `replace`, `remove`, `move`, `clear`, `splice`, and `claim` mutate them; `replace(messageId, newMessage)`, `remove(messageId)`, and `move(messageId, beforeMessageId?)` locate the pending message across both lists. `move` stays inside the current list: omit `beforeMessageId` to append at that list's end, a missing identity or an already-correct position is a no-op, and an anchor from the other list throws. Replacement may change identity and publishes the old message as discarded followed by the new message as inserted. Ordinary removals and `clear()` are durable cancellations and emit `agent/inbox/discarded`. `claim(target)` removes the next proposed batch with pure deletion splices; the loop then emits `agent/inbox/claimed`. `MessageId` is the only occurrence identity and must remain unique while pending.
