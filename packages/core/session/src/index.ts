...
export { formatAbortReason, formatAgentCancelCause, isAgentCancelCause } from './abort-reason.ts'
...
function validateSessionHeader(id: SessionId, input: unknown): SessionHeader {
  ...
  if (record.origin !== undefined && record.origin !== 'subagent' && record.origin !== 'automation') {
    throw new Error('session header origin must be "subagent" or "automation"')
  }
