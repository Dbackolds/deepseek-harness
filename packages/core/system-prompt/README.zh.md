...
- `ctx.systemPrompt.listSections(context?: AssembleContext): RegisteredPromptSection[]`：按拼接顺序列出某一作用域的有效已注册段，并求值每段文本。这是注册表视图，不是完整组装：工具、上下文、assemble waterfall、complete 段还原以及 afterAssemble hook 都不会运行。
