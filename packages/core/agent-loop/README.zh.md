...
每个步骤中，循环会发送针对该 agent 呈现的系统提示词、可见工具 schema 和会话派生消息。它提供 `provider`、`model` 与 `cwd` 变量值，但不添加固定文案。解析到的模型若带有非空 `systemPrompt`，会在插值前替换所有已组装的系统提示词段落；工具 schema 仍留在请求上。
