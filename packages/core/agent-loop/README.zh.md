...
每个步骤中，循环会发送会话的派生消息与可见工具 schema。非空的 `system/message` 节点承载提示词，最新一条是有效版本；空渲染文本会从派生历史中清除所有提示词版本。解析到的模型若带有非空 `systemPrompt`，会在插值前替换所有已组装的系统提示词段落；工具 schema 仍留在请求上。它提供 `provider`、`model` 与 `cwd` 变量值，但不添加固定文案。
