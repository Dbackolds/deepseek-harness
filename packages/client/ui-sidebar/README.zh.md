...
侧边栏外壳插件：负责字标、鲸鱼标识上的未读已完成计数、New Session 操作、其下方的 `sidebar.automation` seat、布局持有的折叠控件、可感知滚动的区域 seat，以及固定在底部的 Settings seat。[ui-automation](../ui-automation/README.md) 占据自动化控件；[ui-workspace](../ui-workspace/README.md) 持有渲染到 `sidebar.workspaces` 的 Workspace 与 Session 浏览器；本包既不派生其中的行，也不持有其视图偏好。折叠到布局拥有的 56px 轨道仍属于本地呈现行为。约定：[slot 系统标准](../../../.agents/notes/implemented/architecture/2026-07-22-slot-type-chain-implementation.md)。
...
`SidebarRootComponentProps` 组合布局 owner share、全局 `useSessions` 和 `useWorkspaces` 钩子、已声明的 `sidebar.automation`、`sidebar.workspaces` 与 `sidebar.settings` 子 slot，以及注入的 `startSession` 与侧边栏切换回调。外壳只为品牌鲸鱼上的未读已完成计数读取 `useSessions`。这里没有插件 store。
...
绿色角标在侧栏展开时贴在字标鲸鱼上，收起时贴在轨道鱼标上。数字是当前列表里仍带有浏览器本地 Completed 提醒的 Session 数；达到 100 时显示为 `99+`。计数上升时，若桌面 Host 注入了 `window.dshDesktop.notifyCompleted`，外壳会调用它；普通浏览器标签页没有该方法，调用为空操作。
...
- **未读已完成计数是本地查看状态**：`SessionManager` 把提醒存在此浏览器；完成时间 > 上次查看时间这一事实永远不会到达宿主。桌面 dock 跳动仅限 macOS，普通浏览器标签页为空操作。
