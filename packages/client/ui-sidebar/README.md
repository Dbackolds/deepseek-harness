...
Sidebar shell plugin: the brand row, New Session action, the `sidebar.automation` list under it, layout-owned collapse control, scroll-aware region seat, and bottom-pinned Settings seat. [ui-automation](../ui-automation/README.md) occupies the first New Session sibling; other plugins may append after it. Occupants inherit New Session capsule geometry from this shell, including a third-party button injected under `[data-dsh-sidebar-actions]` and known DOM entries tagged `data-dsh-mnemon-entry`, `data-dsh-taskboard-entry`, or `data-dsh-ssh-entry`. [ui-workspace](../ui-workspace/README.md) owns the Workspace and Session browser rendered into `sidebar.workspaces`; this package neither derives its rows nor owns its view preferences. Collapse into the layout-owned 56px rail remains presentation-local. Contract: the [slot system standard](../../../.agents/notes/implemented/architecture/2026-07-22-slot-type-chain-implementation.md).
...
`SidebarRootComponentProps` composes the layout owner share, the global `useSessions` and `useWorkspaces` hooks, the declared brand, `sidebar.automation` list, `sidebar.workspaces`, and `sidebar.settings` child slots, and injected `startSession` plus sidebar-toggle callbacks. The shell reads `useSessions` only to forward the unread Completed count to the desktop Host. There is no plugin store.

The count is how many listed Sessions still carry the browser-local Completed reminder. The shell calls `window.dshDesktop.setCompletedUnread(count)` when the desktop Host injected that preload method; a browser tab has no such method and the call is a no-op. The green numeric badge is drawn on the macOS dock whale, not on the in-page brand mark.

During a live collapse, the shell holds the expanded content at its current width while it fades out for 150ms. The upper controls—the shell toggle, New Session, the New Session sibling list, plus add and search rendered through `sidebar.workspaces`—then share one 150ms fade and 49px leftward translation into the 56px rail, ending with the layout's 300ms column slide; every 36px control box follows the same path to the rail's 10px left inset. The bottom-pinned `sidebar.settings` control shares the fade timing but has no horizontal translation. A page that starts collapsed renders the rail statically, and reduced-motion mode disables both transitions.
...
- **The unread Completed count is local viewing state** — `SessionManager` stores the reminder in this browser; completion-time > last-seen never reaches the host. The desktop dock badge and bounce are macOS-only and a no-op in a plain browser tab.
