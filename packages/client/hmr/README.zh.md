...
为通过脚本加载的客户端插件提供热重载。web 组合包无条件挂载该行。自动 rebuilt 广播保持关闭，直到 Host 的 `client-hmr.autoReload` 设置为 true，并且重建 watcher（`pnpm run dev:web`）改写客户端 bundle；轮询仍会跟踪 hash，因此设置页的「重载插件」可以重新计算每一行的 hash。本包持有的通用设置行把该布尔值写入 `$DSH_HOME/settings.yaml`，并对其手动替换发送 `POST /plugins/reload`。

浏览器侧订阅系统 SSE（Server-Sent Events）通道（`GET /plugins/events`），每个 `rebuilt` 或 `reload` 帧重载一个插件，并通过队列串行执行。自动热重载关闭时会忽略 `rebuilt` 帧；`reload` 帧始终执行替换。每帧的顺序是：`invalidate`、`prefetch`（旧 fiber 仍在服务时加载并注册新组合包）、`registry.delete`（在 fiber dispose（资源释放）之前执行：仅 dispose fiber 会触发 vendored Loader 的 self-dispose 分支，把配置项标为禁用）、排空旧 fiber、删除 `entry.fiber`、移除自身拥有的 `<style data-plugin>` 标签、通过 `entry.refresh()` 重新导入并挂载、通过 `fiber.await()` 直接重新抛出启动失败。依赖方由 Cordis 自身重载：fiber 的激活 epoch 会串联其服务提供方的 uid，因此替换提供方 fiber 会级联所有依赖方，无需客户端图分析。node 侧使用一个 interval 检测重建：从同步基线开始 stat-poll 每个图组合包；新增一行后立即重新计算 hash；缺失行保持 dirty；只广播真实 rev 变更。因此，任何生成组合包的 tsdown watch 进程都能触发 HMR（热模块替换），无需 builder→host 通道。
