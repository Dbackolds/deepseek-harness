...
The Host half registers a loopback Connection RPC channel at `/plugin-marketplace` and the `/reload`, `/update`, and `/reboot` commands. The browser half provides `pluginMarketplaceUi` so the shipped Plugins section yields the page. Install accepts one npm registry package name. Path, `file:`, and git specs are refused.
