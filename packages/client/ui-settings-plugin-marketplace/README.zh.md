...
Host 半侧在 `/plugin-marketplace` 注册回环 Connection RPC 通道，以及 `/reload`、`/update`、`/reboot` 命令。浏览器半侧提供 `pluginMarketplaceUi`，使随附的插件分区让出该页。安装只接受一个 npm 注册表包名。路径、`file:` 和 git spec 会被拒绝。
