...
触发器始终存在，因此没有规则的 Host 仍有入口。打开页面时加载一次列表；之后的打开复用快照，直到一次变更或连接重置再次拉取。一张卡片显示规则名称、任务文本、列表镜像里有则附上工作区标题、带剩余时间的时间芯片，以及 `listRuns` 给出的运行次数。启用、停用、立即运行和删除都走模型 tool 与 Host RPC 共用的同一 Host 服务。成功的立即运行会关掉自动化页，并在 Host 列表带上该 Session 后打开它。创建恰好接受一种选择器：一次性延迟、UTC 时刻、至少 300 秒的固定间隔，或带 IANA 时区与可选 ISO 星期的本地时钟。内置模板只预填该表单，提交前不持久化规则。保持唤醒开关写入 `ui-automation.keepAwake`。打开时，存活的宿主持有操作系统休眠断言（macOS 为 `caffeinate -i`，Linux 为 `systemd-inhibit --what=idle`，Windows 为 `SetThreadExecutionState`），关掉开关或卸载插件时释放。助手缺失时不做断言。
