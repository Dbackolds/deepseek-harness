---
description: "Product-update plugin for the Web GUI: Host poller of GitHub Releases, General Settings row, and overlay toast; never downloads or installs."
...
The shipped Web bundle already mounts this plugin. Open **Settings → General** to see the installed version, the last check, and **Check now**. When a newer tag exists and has not been dismissed, an overlay toast offers **Release notes** and **Dismiss**. **Check now** is the only client-initiated poll; a newly opened tab hydrates from the Host cache rather than fetching GitHub itself.

### When to choose it

Mount this plugin when the Web GUI should tell the user a newer GitHub Release exists. Leave it out of a custom profile that has no product channel, or that already owns its own updater. Checking never downloads or installs; if you need an in-app installer, this package is not that surface.
...
A failed poll keeps the last successful result for 24h so a GitHub outage does not blank the row. Without a cache, the row shows **Could not check for updates.** The plugin never writes a release URL that is not `https://github.com/...`.
...
| [`src/checker.ts`](src/checker.ts) | GitHub Releases poller (ETag, body-hash, stale fallback) |
| [`src/client/index.ts`](src/client/index.ts) | Browser half: dictionaries, Settings row, overlay toast |
...
Read these pages when the update row is not enough. They move from this plugin to the Settings shell and the client package map.
