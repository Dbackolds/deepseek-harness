...
`WorkspaceListState.hiddenWorkspaceIds` 以同样方式镜像 Host 的注册表级全局已隐藏 Workspace 集合（列表基线、`hide`/`show` 一元回声、`host/hidden-workspaces-changed`）。已隐藏行仍留在 `items` 中并保留 `sessionIds` 记账；Hide workspace（隐藏工作区）不会清空当前 Session。`startSession` 与 `startInitialSelection` 在当前 Session 所属 Workspace 已隐藏时仍使用该 Workspace，其后才是可见的 No Repo，再其后是最近的可见 Workspace。选择器消费方按隐藏集合过滤 `items`；运行时列表保持完整 Host 顺序。
