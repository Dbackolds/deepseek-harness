...
The section registers the Settings nav row at `settings.section` id `plugins` with `priority: 1`. An out-of-tree marketplace that provides `pluginMarketplaceUi` and registers the same cell at the default priority occupies the page; this package then leaves that nav row and its `configurable` tab empty so the marketplace can declare `settings.plugin.item`. The three host-plane cards still inject into that item slot once the marketplace declares it. The section declares `settings.plugins.tab`, a root list slot whose labels become ordered tabs. It keeps a tab mounted after its first selection, so local drafts and read-only snapshots survive tab switches. The package registers its own `configurable` contribution, which declares the nested `settings.plugin.item` list slot. A plugin that ships a browser half registers its own card into that nested slot and owns its controls; this package neither enumerates namespaces nor renders a form it was not given. Both levels follow the contribution's `order`.
