...
The browser uses HTTP POST for Remote unary calls. API Gateway owns the `/api/remote.mux` WebSocket and its logical streams; in-process compositions provide equivalent Remote streams through `connection.rpc.open` without opening a WebSocket. The Host half owns the sole `/api` route, Fetch bridge, browser authentication, Host/Origin checks, and exact `GET`/`HEAD` route registry. Typert Gateway claims generated Remote endpoints, feature packages register non-JSON responses such as Session-log downloads, and unclaimed requests return 404. Loopback hostname classification remains package-internal to the browser-facing Client state. `ConnectionHandle.api` is optional: API Remotes installs the historical unary face after `llm` and `settings` mount so plugins that still call `connection.api` keep working.
...
- **`connection.api` is a compatibility slot** — Connection does not populate it; API Remotes installs `llm.providers`, `llm.models`, `settings.describe`, and `settings.update` after those namespaces mount.
