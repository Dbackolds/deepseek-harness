...
- **已定稿的 user prompt 在同一会话内编辑**：user 气泡显示时钟、复制和编辑；分支仍只存在于 assistant 回答之下（[决策](../../../.agents/notes/implemented/simplification/2026-08-06-user-bubbles-drop-the-branch-action.md)）。保存已定稿 prompt 会调用 `session.rewrite`，在本会话替换该 prompt 及其后 surface 节点，并从替换消息开启新一轮（[决策](../../../.agents/notes/implemented/feature/2026-08-20-same-session-user-prompt-rewrite.md)）。steering 与待处理气泡仍只有复制；队列编辑仍是待处理 inbox 操作。
