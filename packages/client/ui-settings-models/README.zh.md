...
声明步骤在 `src/onboarding-copy.ts` 中持有完整文案和版本。回环与 `--trusted-host` 浏览器会通过既有 settings API 比较并写入 `ui-onboarding.welcomeNoticeVersion`；只有明确点击「继续」才会记录当前版本。未声明的远程页面无法调用这些 RPC，因此「继续」会失败，重载后声明会再次出现。
