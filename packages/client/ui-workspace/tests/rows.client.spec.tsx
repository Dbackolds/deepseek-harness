...
describe('workspace browser rows', () => {
  it('omits only an empty leading status slot in the hierarchy-free flat list', () => {
    const idle: SessionNode = {
      ...
      interrupted: false, runningSubagentCount: 0, completed: false, hasActiveSchedule: false, updatedAt: 0,
    }
  ...
  it.each([
    ...
    const result: SearchResultNode = {
      ...
      pendingInteraction, running: true, interrupted: false, runningSubagentCount: 0, completed: false, hasActiveSchedule: false,
    }
    ...
    const renderRow = (over: Partial<SessionNode>) => render(
      <SessionNodeItem
        node={{
          ...
          interrupted: false, runningSubagentCount: 0, completed: false, hasActiveSchedule: false, updatedAt: 0, ...over,
        }}
  ...
  it('shows descendant activity without describing an idle parent as running', () => {
    ...
    try {
      const node: SessionNode = {
        ...
        interrupted: false, runningSubagentCount: 2, completed: false, hasActiveSchedule: false, updatedAt: 0,
      }
  ...
  it('keeps descendant activity secondary while the parent is running', () => {
    ...
    try {
      const node: SessionNode = {
        ...
        interrupted: false, runningSubagentCount: 1, completed: false, hasActiveSchedule: false, updatedAt: 0,
      }
      render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} t={t} />)
      const row = screen.getByRole('treeitem')
  ...
  it('keeps child activity as a secondary status while user attention is primary', () => {
    const node: SessionNode = {
      ...
      running: false, interrupted: false, runningSubagentCount: 1, completed: false, hasActiveSchedule: false, updatedAt: 0,
    }
    render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
      onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} t={t} />)
    const row = screen.getByRole('treeitem')
  ...
  it('shows the error dot on an interrupted session row', () => {
    const node: SessionNode = {
      id: sid('crash'), title: 'Crashed', blank: false, running: false,
      interrupted: true, runningSubagentCount: 0, completed: false, hasActiveSchedule: false, updatedAt: 0,
    }
    render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
      onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} t={t} />)
  ...
  it('shows the green done dot on a finished search result row', () => {
    render(<SearchResultItem
      result={{
        ...
        interrupted: false, runningSubagentCount: 0, completed: true, hasActiveSchedule: false,
      }}
  ...
  it('workspace hover card shows a POSIX home descendant as ~ and still copies the full path', async () => {
    ...
    try {
      const group: GroupNode = {
        ...
        folders: [], sessionCount: 0, expanded: false, containsCurrent: false, sessions: [],
      }
  ...
  it.each([
    ...
    try {
      const node: SessionNode = {
        ...
        pendingInteraction, running: true, interrupted: false, runningSubagentCount: 0,
        completed: false, hasActiveSchedule: false, updatedAt: 0,
      }
  ...
  it('completed hover card shows the Completed status line', () => {
    ...
    try {
      const node: SessionNode = {
        ...
        interrupted: false, runningSubagentCount: 0, completed: true, hasActiveSchedule: false, updatedAt: 0,
      }
      render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} t={t} />)
      fireEvent.pointerEnter(screen.getByRole('treeitem').parentElement as HTMLElement)
