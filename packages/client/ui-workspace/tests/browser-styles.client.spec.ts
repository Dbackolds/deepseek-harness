...
describe('WorkspaceBrowser.module.css list', () => {
  ...
  it('keeps 2px between rows and 8px between activity and workspace groups', () => {
    ...
    expect(declarations('.activityCount')?.get('border-radius')).toBe('999px')
    expect(declarations('.activityCount_unread')?.get('background')).toBe('var(--dsw-alias-state-success-primary)')
    expect(declarations('.activityCount_running')?.get('background')).toBe('var(--dsw-alias-state-business-primary)')
    expect(declarations('.activityCount_abnormal')?.get('background')).toBe('var(--dsw-alias-state-error-primary)')
    expect(declarations('.activityChevronOpen')?.get('transform')).toBe('rotate(90deg)')
    expect(declarations('.activityBody')?.get('display')).toBe('grid')
    expect(declarations('.activityBodyCollapsed')?.get('grid-template-rows')).toBe('0fr')
  })
