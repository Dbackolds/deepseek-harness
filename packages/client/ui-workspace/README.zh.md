...
该浏览器通过全局运行时钩子将 Session 行渲染为分组或扁平形式，并负责 Workspace 添加／重命名／重排序以及 Session 重排序。可见 Session 在两种呈现下都拆成四个状态分区：**已完成**容纳自那次完成后焦点尚未离开的 Session，**运行中**容纳待处理交互或自身／后代活动，**异常**容纳上次被崩溃／重载中断且尚未再次运行的 Session，**历史记录**容纳其余空闲 Session。已完成、运行中和异常的标题旁有彩色数量徽章。每个分区标题都有下拉箭头，并用高度动画折叠行；历史记录在展开时仍使用五行溢出控件。每个 Workspace 会记住自身是关闭还是显示 Session；打开后会显示未折叠的已完成、运行中和异常行，历史记录默认五条。从 Workspace 行创建 Session 时会先打开该分组。新的空白 Session 在首次获受理的 prompt 之前不进入列表，发送后出现在运行中。Workspace 列表基线就绪后，浏览器持久化的展开状态与 Session 顺序记录只保留当前 Workspace id、Ungrouped 和单列表记账。视图选项把分组方式和每个记账各自的一份浏览器持久化 Session 顺序放在一起：真实 Workspace 从 `WorkspaceView.sessionIds` 初始化，Ungrouped 和跨 Workspace 的单列表则从最近更新时间顺序初始化。**手动排序**和**最近更新**在两种呈现方式下都可用。进入最近更新时会执行一次完整的时间排序，后续 user prompt 或 steer 会将对应 Session 置顶一次；进入手动排序则保留所有当前位置并停用后续置顶。两种模式下的拖拽都会编辑当前顺序；真实 Workspace 在手动模式下的拖拽还会更新 Host Session 记账，而 Ungrouped 和单列表因没有单一 Workspace 记账，其顺序始终只保存在浏览器本地。单列表没有父级层次，因此不显示空的左侧状态槽；Session 存在可见状态时仍保留该槽。无论采用哪种 Session 顺序，Workspace 拖拽顺序都由 Host 持久化。
