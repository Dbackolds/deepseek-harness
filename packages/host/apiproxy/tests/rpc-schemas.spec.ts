...
describe('rpcErrorSchema', () => {
  it('accepts every code branch with its required details', () => {
    ...
    expect(rpcErrorSchema.parse({ code: 'session-rehome-no-repo', message: 'm', details: { path: '/x' } }).code).toBe('session-rehome-no-repo')
...
describe('sessions domain schemas', () => {
  ...
  it('validates the per-method request/value pairs', () => {
    ...
    expect(sessionUpdateQueueRequestSchema.parse({
      sessionId: 's1', itemId: 'i1', action: { kind: 'move', beforeItemId: 'i2' },
    }).action).toEqual({ kind: 'move', beforeItemId: 'i2' })
    expect(sessionUpdateQueueRequestSchema.parse({
      sessionId: 's1', itemId: 'i1', action: { kind: 'move' },
    }).action).toEqual({ kind: 'move' })
    expect(() => sessionUpdateQueueRequestSchema.parse({
