...
function fakeApi(overrides: Partial<{ muxFrames: MuxFrame[]; hostFrames: HostFrame[]; crashOn: string }> = {}): ApiProxy {
  ...
  return {
    ...
    workspace: {
      async list(request) {
        return { rpcId: request.rpcId, result: { ok: true, value: { items: [], archivedSessionIds: [], hiddenWorkspaceIds: [] } } }
      },
      async create(request) {
        return {
          rpcId: request.rpcId,
          result: { ok: true, value: { workspace: { workspaceId: 'w1' as never, path: '/w', folders: [], title: 'w', sessionIds: [], createdAt: 't', updatedAt: 't' }, created: true } },
        }
      },
      async rename(request) {
        return {
          rpcId: request.rpcId,
          result: { ok: true, value: { workspace: { workspaceId: 'w1' as never, path: '/w', folders: [], title: 'w', sessionIds: [], createdAt: 't', updatedAt: 't' } } },
        }
      },
      ...
      async show(request) {
        return { rpcId: request.rpcId, result: { ok: true, value: { hiddenWorkspaceIds: [] } } }
      },
      async addFolder(request) {
        return {
          rpcId: request.rpcId,
          result: { ok: true, value: { workspace: { workspaceId: 'w1' as never, path: '/w', folders: [request.payload.path], title: 'w', sessionIds: [], createdAt: 't', updatedAt: 't' } } },
        }
      },
    ...
    git: {
      describe(request: RpcRequest<{ sessionId?: string; workspaceId?: string }>) {
        return Promise.resolve({
          rpcId: request.rpcId,
          result: { ok: true as const, value: { currentBranch: 'main', detached: false, worktreePath: '/w', isolated: false, dirtyCount: 0, unpushedCount: 0, branches: [] } },
        })
      },
      checkout(request: RpcRequest<{ sessionId: string; branch: string }>) {
        return Promise.resolve({
          rpcId: request.rpcId,
          result: { ok: true as const, value: { currentBranch: request.payload.branch, detached: false, worktreePath: '/w', isolated: false, dirtyCount: 0, unpushedCount: 0, branches: [] } },
        })
      },
      createBranch(request: RpcRequest<{ sessionId: string; branch: string }>) {
        return Promise.resolve({
          rpcId: request.rpcId,
          result: { ok: true as const, value: { currentBranch: request.payload.branch, detached: false, worktreePath: '/w', isolated: true, dirtyCount: 0, unpushedCount: 0, branches: [] } },
        })
      },
    },
    agentPresets: {
    ...
    skills: {
      ...
      async catalog(request) {
        return { rpcId: request.rpcId, result: { ok: true, value: { skills: [{ name: 'commit-helper', description: 'Git commits', modelInvocable: true, userInvocable: true, source: 'bundled', provider: 'fixture' }] } } }
      },
    },
    automation: {
      async list(request) {
    ...
    systemPrompt: {
      async list(request) {
        return { rpcId: request.rpcId, result: { ok: true, value: { sections: [] } } }
      },
    },
    events: {
...
describe('unary round trip (handler ⇄ client, no network)', () => {
  ...
  it('round-trips skill.catalog through the wire form', async () => {
    const c = client()
    const skills = await c.skills.catalog({})
    expect(skills.result).toEqual({
      ok: true,
      value: { skills: [{ name: 'commit-helper', description: 'Git commits', modelInvocable: true, userInvocable: true, source: 'bundled', provider: 'fixture' }] },
    })
  })
