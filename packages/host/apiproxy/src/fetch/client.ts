...
import { systemPromptListValueSchema } from '../api/system-prompt.schema.ts'
import {
...
export interface IApiClient {
  ...
  systemPrompt: {
    list(payload: RequestPayload<'systemPrompt.list'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'systemPrompt.list'>>>
  }
...
const UNARY_VALUE_SCHEMAS: { [K in keyof RpcMethodMap]: z.ZodType<Wire<ResponseValue<K>>> } = {
  ...
  'systemPrompt.list': systemPromptListValueSchema,
}
...
export abstract class AbstractApiClient implements IApiClient {
  ...
  readonly systemPrompt: IApiClient['systemPrompt'] = {
    list: (payload, signal) => this.callUnary('systemPrompt.list', payload, signal),
  }
