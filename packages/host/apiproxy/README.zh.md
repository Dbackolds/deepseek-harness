...
`git.describe`、`git.checkout` 与 `git.createBranch` 领域列出工作区仓库并切换一个会话的 overlay。工作区检出目录仍是 Session 成员关系键。检出工作区 HEAD 以外的任何名字都会在 `$DSH_HOME/worktrees/<workspace-id>/<session-id>` 下创建或复用一个关联 worktree，并记录一条 `git/worktree` 事件，因此两个会话可以停在不同分支上。本地头使用短名；远程跟踪行保留远程前缀。工作区 HEAD 处于 detached、且只匹配一个已列出名字时报告该名字；同一提交上有多个名字时报告短对象 id。快照还报告这个会话 worktree 的未提交路径数，以及相对上游的未推送提交数（没有上游时为 0）。不是 Git 仓库的路径回答 `git-not-a-repository`；非法或重复的名字回答 `git-branch-invalid` 或 `git-branch-exists`。
