...
`reportDelivery` 为每条已接受的报告选择父级调度方式。`wakeup`（默认值）会唤醒停驻的父级，并开启一个普通的后续轮次。繁忙父级在发送时遵循 Host `subagent-delivery.reportBusy`：`steer`（默认）把报告准入到最近的后续 step；`queue` 则开启后续一轮。Quiet 投递仍走注入且永不唤醒。`quiet` 使用 `parent.inject()`，在不启动父级模型请求的情况下添加面向模型的上下文：父级空闲时，追加操作会在调用返回前完成；报告到达正在准入或运行的父级时，则会暂存到下一个安全日志位置。Quiet 仍是部署调度策略，因此面向模型的 schema 不能在单次调用中选择或覆盖该策略。
...
每次调用都会在执行上报的子级中产生一条简短确认消息。父级还会为上报内容支付 token 成本：静默投递把它加入父级的下一次请求；唤醒投递在空闲父级上是后续一轮；繁忙父级遵循 Host `subagent-delivery.reportBusy`。
...
仅追加；报告位于父级可复用请求前缀之后。静默投递不启动请求。唤醒投递在空闲父级上开启后续一轮；繁忙父级遵循 Host `subagent-delivery.reportBusy`。
