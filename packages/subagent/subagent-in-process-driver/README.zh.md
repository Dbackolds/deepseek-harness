...
除非 `options.cwd` 提供已解析的覆盖值，否则子 agent 继承父工作目录；除非 `request.agentOptions` 覆盖，否则还会继承父 agent 的提供方、模型、输出 token 上限，以及相同路由上的推理强度。子 agent 更换提供方或模型时，不会继承上一模型的不透明推理强度。它获得全新的扁平注册作用域：父级所有权不会导入父 agent 的工具限制，也不会建立权限子集。
...
`InProcessRunOptions` 的形态为 `{ seed?: SessionEvent[]; cwd?: string }`。spawn 两者都省略。fork 提供已配平的已完成轮次前缀，并记录其长度，确保结果读取器不会把作为初始内容的父 agent 消息误认为子 agent 输出。已准备好隔离检出的提供方设置 `cwd`；省略则继承父会话目录。
