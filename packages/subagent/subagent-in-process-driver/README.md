...
The child inherits the parent working directory unless `options.cwd` supplies a resolved override, and inherits the parent provider, model, output-token cap, and same-route reasoning effort unless `request.agentOptions` overrides them. A child that changes provider or model does not inherit the previous model's opaque effort id. It gets a fresh flat registration scope: parent ownership does not import parent tool restrictions or establish an authority subset.
