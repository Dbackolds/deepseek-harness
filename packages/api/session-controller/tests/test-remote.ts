...
export interface TestSessionRemote {
  ...
  rehome(request: SessionRehomeRequest): Promise<RemoteResult<SessionRehomeValue>>
  rewrite(request: SessionRewriteRequest): Promise<RemoteResult<SessionRewriteValue>>
...
export function createSessionTestRemote(
  ...
  return {
    ...
    rehome: request => remoteResult(() => direct.rehome(request)),
    rewrite: request => remoteResult(() => direct.rewrite(request)),
