  ...
  sessionRemotes(): RuntimeRemotes {
    return {
      ...
      session: {
        ...
        rehome: payload => this.record('session.rehome', payload, Promise.resolve(ok({ cwd: '/f/ws' } as never))),
        rewrite: payload => this.record('session.rewrite', payload, Promise.resolve(ok({ accepted: true }))),
      ...
      workspace: {
        ...
        unarchiveSession: payload => this.record('workspace.unarchiveSession', payload, Promise.resolve(ok({ archivedSessionIds: [] }))),
        addFolder: payload => this.record('workspace.addFolder', payload, Promise.resolve(ok({ workspace: fakeWorkspace('fk-ws') }))),
        removeFolder: payload => this.record('workspace.removeFolder', payload, Promise.resolve(ok({ workspace: fakeWorkspace('fk-ws') }))),
        hide: payload => this.record('workspace.hide', payload, Promise.resolve(ok({ hiddenWorkspaceIds: [] }))),
        show: payload => this.record('workspace.show', payload, Promise.resolve(ok({ hiddenWorkspaceIds: [] }))),
