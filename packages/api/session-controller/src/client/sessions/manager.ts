...
export class SessionManager {
  ...
  */
  ...
  /**
   * Rewrite a settled user prompt in this same session and keep the source selected.
   * @param opts - source session, current-surface user-message seq, and replacement content.
   */
  async rewrite(opts: {
    sessionId: SessionId
    atSeq: number
    content: import('../../types.ts').PromptContentPart[]
  }): Promise<void> {
    const session = this.sessions.get(opts.sessionId)
    if (session === undefined) throw new Error(`sessions.rewrite: unknown session ${opts.sessionId}`)

    const result = await session.rewrite(opts.atSeq, opts.content)
