...
import type { AttachmentIdType, FileAttachmentRef, ImageAttachmentRef, VideoAttachmentRef } from '@deepseek-ai/dsh-attachment'
...
export class Session implements SessionFace {
  ...
  ): Promise<RemoteResult<{ attachment: ImageAttachmentRef | VideoAttachmentRef; data: Uint8Array }>> {
   ...
   * Subagent addresses cannot rewrite history.
   * @param atSeq - current-surface user/message seq being edited.
   * @param content - replacement text plus browser-owned temporary uploads.
  ...
  async rewrite(
    ...
    if (this.address !== undefined) {
      ...
      result = await this.remote.session.rewrite({
        sessionId: this.sessionId,
        atSeq,
        content,
        clientTimeZone: resolvedClientTimeZone(),
