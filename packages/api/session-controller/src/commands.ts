...
export class SessionCommandController {
   ...
   * @param request - Session identity, target seq, and replacement content.
  ...
  async rewrite(request: SessionRewriteRequest): Promise<SessionRewriteValue> {
    ...
    const admit = async (): Promise<SessionRewriteValue> => {
      try {
        if (hasImage) {
  ...
  /**
   * Move this Session's effective home and workspace account to an existing directory.
   * @param request - Session identity and destination directory.
   * @returns the destination workspace projection.
   */
  async rehome(request: SessionRehomeRequest): Promise<SessionRehomeValue> {
    const agent = await this.resolveAgent(request.sessionId)
    ...
    for (const workspace of this.ctx.workspaceRegistry.list()) {
      if (workspace.id !== target.id && workspace.sessionIds.includes(request.sessionId)) {
