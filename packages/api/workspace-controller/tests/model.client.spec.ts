...
class FakeWorkspaceRemote implements WorkspaceRemote {
  ...
  addFolder(request: { workspaceId: WorkspaceId; path: string }): Promise<RemoteResult<WorkspaceValue>> {
    this.record('addFolder', request)
    return Promise.resolve(remoteOk({ workspace: workspace(String(request.workspaceId)) }))
  }

  removeFolder(request: { workspaceId: WorkspaceId; path: string }): Promise<RemoteResult<WorkspaceValue>> {
    this.record('removeFolder', request)
    return Promise.resolve(remoteOk({ workspace: workspace(String(request.workspaceId)) }))
  }

  hide(request: { workspaceId: WorkspaceId }): Promise<RemoteResult<{ hiddenWorkspaceIds: readonly WorkspaceId[] }>> {
    this.record('hide', request)
    return Promise.resolve(remoteOk({ hiddenWorkspaceIds: [request.workspaceId] }))
  }

  show(request: { workspaceId: WorkspaceId }): Promise<RemoteResult<{ hiddenWorkspaceIds: readonly WorkspaceId[] }>> {
    this.record('show', request)
    return Promise.resolve(remoteOk({ hiddenWorkspaceIds: [] }))
  }
...
function baseline(
  ...
  model.replaceBaseline({ items, archivedSessionIds, hiddenWorkspaceIds: [] })
}
