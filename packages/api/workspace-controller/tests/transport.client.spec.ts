...
describe('Workspace state stream', () => {
  it('delivers one baseline followed by increments', async ({ mock, start }) => {
    ...
    const increments: WorkspaceFollowFrame[] = [
      ...
      { type: 'hidden', hiddenWorkspaceIds: [view.workspaceId] },
    ]
    ...
    const replaceHidden = vi.fn<WorkspaceFollowSink['replaceHidden']>()
    const stream = createWorkspaceStateStream(remote, {
      accept: accepts({ replaceBaseline, upsertView, removeView, replaceOrder, replaceArchived, replaceHidden }),
      failed: vi.fn(),
    })

    stream.start()
    stream.start()
    await vi.waitFor(() => { expect(replaceHidden).toHaveBeenCalledOnce() })
    ...
    expect(replaceHidden).toHaveBeenCalledWith([view.workspaceId])
