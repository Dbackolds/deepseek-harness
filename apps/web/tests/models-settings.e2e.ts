...
describe('web e2e: Models settings page configures a dormant provider', () => {
  ...
  it('opens the add card over the dormant directory vocabulary', async () => {
    ...
    // FAC is composition-owned, so the page opens with its collapsed setup
    // card. The remaining catalog stays addable.
  ...
  it('declares a route the adapter does not ship', async () => {
    ...
    await dialog.getByLabel('支持图片 1').check()
    ...
    expect(document).toContain('input:')
    expect(document).toMatch(/-\s+text/)
    expect(document).toMatch(/-\s+image/)
