...
describe('web e2e: navigation & panes over a rich seeded session', () => {
  ...
  beforeEach(async () => {
    ...
    // counts; the Chat bucket row is the barrier).
    await page.getByText('Chat', { exact: true }).waitFor({ timeout: 30_000 })
  }, 120_000)
  ...
  it.skipIf(MODE === 'record')('finds an unopened seeded session by message content and opens it', async () => {
    ...
    // seeded Chat bucket row is the final user-visible barrier before
  ...
  it.skipIf(MODE === 'record')('downloads through the Session Header and /export with one dialog', async () => {
    ...
    await observer.getByText('Chat', { exact: true }).waitFor({ timeout: 30_000 })
