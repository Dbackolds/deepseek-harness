...
// credential is absent, the welcome notice is the only first-run dialog, and
// the Models-page key write lands in an isolated harness home without a
// reload or model call.
...
describe.skipIf(MODE === 'record')('web e2e: first-run DeepSeek credential setup', () => {
  ...
  it('stores a key write-only and observes configured state without restarting', async () => {
    ...
    // The Models page is the only place a missing official key is entered.
    ...
    const edit = settings.getByRole('button', { name: '编辑 DeepSeek (deepseek-official)' })
    ...
    // and still no credential dialog follows it.
  ...
  it('never paints the takeover chrome on a configured reload, even with the settings join held open', async () => {
    ...
    // Regression pin for the reload flash: the welcome step is satisfied, yet
    // it must load private facts before deciding not to show. Dialog chrome
    // lives inside the visible branch, so the deciding window paints and
    // blocks nothing. Holding settings.describe widens that window from loopback
