...
describe('web e2e: sidebar Automation under New Session', () => {
  ...
  it('opens the panel under 新会话 and creates an after rule', async () => {
    ...
    await trigger.click()
    const dialog = page.getByRole('dialog', { name: '自动化' })
    await dialog.waitFor({ timeout: 10_000 })
    await dialog.getByRole('button', { name: '新建规则' }).click()
    await dialog.getByPlaceholder('新会话要执行的任务').fill('ping from automation e2e')
    await dialog.getByLabel('延迟秒数').fill('3600')
    ...
    await expect.poll(() => dialog.getByText('ping from automation e2e').count(), { timeout: 10_000 }).toBe(1)
