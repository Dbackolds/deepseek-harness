...
The production Web runner needs built package and frontend artifacts (`pnpm run build`). It serves `http://127.0.0.1:3080` by default. `dsh desktop` opens the Electron window around that same Host, reusing the last successful loopback port unless `--port` is forwarded, and falling back to an OS-chosen port when none is remembered or that port is occupied. The CLI intentionally does not support `--host 0.0.0.0` yet and exits with a usage error; `--trusted-host` adds named authorities accepted by the `/api` browser-trust fence.
