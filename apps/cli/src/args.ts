 ...
 * window around that same profile; `plugin` manages a profile's plugin
...
/** Open the Electron window around the web profile. */
interface DesktopInvocation {
  mode: 'desktop'
  /** Extra arguments forwarded to the web Host the window wraps. */
  args: string[]
}
...
Examples:
  ...
  dsh desktop                                open the desktop window around dsh web
