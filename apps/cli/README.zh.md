...
生产运行需要已构建的包与前端产物。请在仓库根目录单独运行 `pnpm run build`，然后使用 `pnpm dsh <args...>` 运行 TypeScript 入口并转发所有参数；模块解析约定以[源码执行参考](reference/README.md#source-execution)为准。`pnpm desktop` 或 `pnpm dsh desktop` 会打开包裹同一套已构建 Host 的 Electron 窗口。可下载安装包是独立的 `desktop-v*` GitHub Release（[desktop 包](../desktop/README.md)）。
