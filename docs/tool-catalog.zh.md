...
| `@deepseek-ai/dsh-tool-fs` | `edit`、`read`、`read_image`、`read_video`、`write` | `ctx.tools`、`ctx.fs`、`ctx.systemPrompt`、`ctx.attachments (media-tool registration)`、`ctx.llm + an image/video-capable route (media-tool execution)` | `tool/call`、`fs/write-intent or fs/edit-intent for mutations`、`fs/observed after read presence/absence or successful file operation`、`durable attachment (read_image/read_video)`、`tool/result` | - | 先读后写／编辑策略由 `@deepseek-ai/dsh-fs-observation-policy` 添加；它是一个 `fs/*` 事件门禁插件，不会改变 schema。加载这些工具的部署按预期也应加载该插件。没有 `ctx.attachments` 时图片与视频工具不会注册；其 schema 与路由无关，执行时除非确切路由的模型声明图片（或视频）输入，否则拒绝。 |
...
仅在规划模式下使用。提交计划供用户评审，并在获批后退出规划模式。发送**完整的** Markdown 计划，以一个为计划命名的 # 标题开头。用户可以批准（在本轮次的下一步骤执行计划），也可以要求继续规划；其反馈会通过工具结果返回，请修改后再次提交。
...
```json
{
  ...
  "properties": {
    ...
    "path": {
      "type": "string",
      "description": "File or directory to search. Defaults to every folder in the session workspace; a relative path resolves against the current working directory."
    },
    "include": {
      "type": "string",
...
归档一个对话，使其离开所有分组视图。会话日志和 workspace 记账席位保留。已归档的 id 是成功且不写入的空操作。
...
把一个对话的有效家和侧栏分组迁到已存在目录。不要 mkdir。规范 No Repo 目录会被拒绝。未注册的已存在目录会先注册。跨组移动用这个工具；同组顺序用 session_control_reorder。
...
重命名任意逻辑会话，并把标题钉住以免被自动生成覆盖。与侧栏写入的用户来源 session/title 事件相同。空标题或纯空白标题会失败。subagent 所有的会话会失败。冷会话在可用时由 Host session.rename 恢复。
...
在当前 workspace 顺序内移动一个已记账对话。省略 before_session_id 则追加到末尾。不改工作目录。未分组对话必须先改挂。
...
把一个已归档对话恢复到原先的分组席位。已知且未归档的 id 是成功且不写入的空操作。这不会打开该对话。
...
列出用于对话分组的已注册 workspace：id、标题、路径、隐藏标志，以及已记账会话 id（省略已归档对话）。在改挂或调序前先调用它。
...
搜索、停止和发送是 `ctx.sessionControl` 上的薄适配器。归档、取消归档、改挂、调序和工作区列表等待 `ctx.workspaceRegistry`（Web 组合会挂载；CLI/TUI 不会）。改挂在存在 `ctx.apiProxy` 时优先走 Host `session.rehome`。
