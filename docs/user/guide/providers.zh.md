...
因此自定义提供方下的视觉模型需要一条声明。在模型页打开该提供方，找到对应模型行，勾选**支持图片**。这会为该模型写入 `input: [text, image]`。同一字段仍可直接写在 `$DSH_HOME/settings.yaml` 中：
...
- **图片在发送前被拒绝**：该模型未声明图片模态。请在**设置 → 模型**中勾选该模型的**支持图片**，或在 `$DSH_HOME/settings.yaml` 里给自定义提供方的模型加上 `input: [text, image]`；DeepSeek 自身的 chat-completions 路由是纯文本的，且无法通过配置改变。

- **密钥与地址都正确，网关却拒绝每一个请求**：它的请求形状与 OpenAI 不同。先在路由上设 `compat.supportsDeveloperRole: false` 与 `compat.maxTokensField: max_tokens`。
- **只有推理模型失败**：pi-ai 把它们的系统提示词以 `developer` 角色发出，而网关拒绝该角色。设 `compat.supportsDeveloperRole: false`。已经写了 `thinkingFormat: zai` 的模型会自动填入该 `false`，不必再写一遍。
