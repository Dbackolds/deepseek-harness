...
| session | 会话 | | | |
| session event | 会话事件 | | | |
| setup card | 设置卡片 | | | 首次运行时代替行卡出现的配置卡，表单默认收起 |
...
| Hide workspace | 隐藏工作区 | | 归档工作区、删除工作区 | 不拆会话账本的 Host 持久显示折叠；删除仍是注销登记 |
| Show workspace | 显示工作区 | | 取消隐藏、恢复、取消归档 | 从隐藏集合移除并按原持久化序号回到主列表 |
| Hidden | 已隐藏 | | 归档、回收站、未分组 | 侧边栏末尾容纳隐藏工作区的分区 |
