...
开发环境下，[dsh-client-hmr](../../packages/client/hmr/README.zh.md) 是注册表的监视驱动：它的 Node 半从同步取得的基线出发，对图中每一行的 bundle 做 stat 轮询，变化时调用 `rebuilt(id)`，并经 `onGraphChanged` 重新同步监视集合。自动 SSE（Server-Sent Events）`rebuilt` 帧保持关闭，直到 `client-hmr.autoReload` 为 true；`POST /plugins/reload` 会对同一批监视重新哈希并广播 `reload` 帧。生产环境的图完全不含 HMR（热模块替换）行；模块宿主自身从不监视文件。
