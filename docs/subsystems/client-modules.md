...
In development, [dsh-client-hmr](../../packages/client/hmr/README.md) is the registry's watch driver: its node half stat-polls every graph row's bundle from a synchronously captured baseline, calls `rebuilt(id)` on change, and resyncs its watch set through `onGraphChanged`. Automatic SSE `rebuilt` frames stay off until `client-hmr.autoReload` is true; `POST /plugins/reload` re-hashes the same watches and broadcasts `reload` frames. Production graphs omit the HMR row entirely; the module host itself never watches files.
