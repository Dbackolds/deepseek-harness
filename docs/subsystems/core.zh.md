...
在 `agent/request` 之后，分发要求 `provider` 与 `model` 都存在。提供 `maxTokens` 时，它必须是正安全整数，并限制每次对话模型请求的输出；省略时，系统会在写入请求 header 前填入确切模型的适配器默认值，否则提供方行为保持不变。提供 `reasoningEffort` 时，它必须是非空的适配器所有标识，并随每次对话模型请求发送；省略时由适配器或提供方默认值控制。agent 作用域的 `deployment:persona` 提示词段落可以遮蔽全局默认 persona。
